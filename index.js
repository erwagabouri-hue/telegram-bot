...
if (majorLeagues.indexOf(f.league && f.league.id) !== -1 && f.fixture && f.fixture.status
target = f;
break;
}
}
...
const goals = (topScorer.statistics && topScorer.statistics[0] && topScorer.statistics[0].g
...
const LIMIT_MSG = "Limite journaliere atteinte\n\nTu as utilise tes 2 scans gratuits d'aujour
...
if (!picks.length) return ctx.reply("Aucun value bet detecte pour les prochaines 48h. Ree
...
incrementUsage(userId);
if (!picks.length) return ctx.reply("Aucun value bet basket detecte. Reessaie plus let text = "TOP VALUE BET IA - BASKET\nProchaines 48h\n\n";
...
incrementUsage(userId);
const allPicks = [];
footPicks.forEach(function(p) { allPicks.push({ sport: "FOOT", home: p.home, away: p.away
basketPicks.forEach(function(p) { allPicks.push({ sport: "BASKET", home: p.home, away: p.
...
const userId = ctx.from.id;
const check = canScan(userId);
const txt = check.premium ? "Premium - Scans illimites" : "Gratuit - " + check.remaining +
